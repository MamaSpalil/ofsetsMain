...
│   │   ├── GameActionMonitor.h/.cpp   # Монитор ВСЕХ игровых действий
...
- [x] **GameActionMonitor** — полный мониторинг ВСЕХ игровых действий
  - **Клавиатура**: отслеживание всех нажатий клавиш (C, M, V, I, S, Enter, Escape, F1-F12, 0-9, и т.д.)
  - **HP/MP**: изменения здоровья и маны с подсчётом разницы
  - **Уровень**: повышение уровня персонажа
  - **Опыт**: получение опыта
  - **Zen**: подбор и трата валюты
  - **Убийства**: убийства игроков и монстров
  - **Карты**: смена локаций, телепортация, MapList
  - **UI**: открытие/закрытие инвентаря, окна персонажа, навыков, чата
  - **Характеристики**: изменения STR, AGI, VIT, ENE
  - **Смерти**: отслеживание гибели персонажа
  - **Сцены**: смена экранов (логин, выбор персонажа, игра)
  - **Формат логирования**:
    ```
    ...
    ```
  - **База данных**: MuTrackerDB.csv — перезаписывается при каждом запуске
  - Периодический сброс данных на диск (каждые 30 секунд)
...
- [x] **SharedMemory IPC** — обмен данными между DLL и Loader
  ...
  - Таблица игровых событий (GameActionEventEntry)
